package net.democracycraft.elections.src.service;
...
public class SqlElectionsService implements ElectionsService {
    ...
    public void runDeletedPurgeSweep(int retentionDays) {
        ...
        for (ElectionEntity e : schema.elections().getAll(null)) {
            if (e == null) continue;
            if (!net.democracycraft.elections.src.data.ElectionStatus.DELETED.name().equals(e.status)) continue;
            ...
            for (StatusChangeEntity sc : changes) {
                if (sc != null && sc.type != null && sc.type.equalsIgnoreCase(net.democracycraft.elections.src.data.StateChangeType.DELETED.name())) {

...
public class CandidateVoteMenu extends ChildMenuImp {
    ...
    private final ItemStack candidateHead;
    private final Elections plugin;

    public CandidateVoteMenu(Player player, ParentMenu parent, ElectionsService electionsService, int electionId, int candidateId, ItemStack candidateHead, Elections plugin) {
        super(player, parent, "ballot_cand_" + electionId + "_" + candidateId);
        this.electionsService = electionsService;
        this.electionId = electionId;
        this.candidateId = candidateId;
        this.candidateHead = candidateHead;
        this.plugin = plugin;
    ...
    public static class Config implements Serializable {
        ...
        public String saveBtn = "<green>Save</green>";
    ...
    private Dialog build() {
        ...
        if (candidateHead != null && candidateHead.getType() != Material.AIR) {
            dialogBuilder.addBody(DialogBody.item(candidateHead).showTooltip(true).build());
        }
        ...
        if (system == VotingSystem.BLOCK) {
            ...
            dialogBuilder.button(miniMessage(config.saveBtn), context -> {
                ...
                new CandidateVoteListMenu(context.player(), getParentMenu(), electionsService, electionId, plugin).open();
            });
        } else {
            ...
            dialogBuilder.button(miniMessage(config.saveBtn), context -> {
                ...
                if (value == null || value < 1 || value > maxRank) {
                    ...
                    new CandidateVoteMenu(context.player(), getParentMenu(), electionsService, electionId, candidateId, candidateHead, plugin).open();
                    return;
                }
            ...
            dialogBuilder.button(miniMessage(config.clearRankBtn), context -> { session.clearRank(candidateId); new CandidateVoteListMenu(context.player(), getParentMenu(), electionsService, electionId, plugin).open(); });
        }
        ...
        dialogBuilder.buttonWithPlayer(miniMessage(config.backBtn), null, (playerActor, response) -> {
            ...
            new CandidateVoteListMenu(playerActor, getParentMenu(), electionsService, electionId, plugin).open();
        });
        return dialogBuilder.build();
    }
}
